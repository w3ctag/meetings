...
Mark: there's also a newTunnel Protocol for Connect spec [header name is ALPN]
...
…We can't take just any work into the HTML WG, or especially the Web App WG, with a more restrictive charter.
...
Alex: but if w3c is slow, it should be more good

Brian:  Can we use custom element demos as a way toward standards?

DKA:  Yes, for some things. But there are other things which you cannot achieve using

Mike: Example is the picture element — tis waa soemthing people wanted — responsive pictures — that came from developers.
...
Alex: If you don’t have implementers involved then your ideas are toast. So having a single CG is interesting place to connect with implemnters. There can be push and pull - the CG could look at news from frameworks fetures..
...
Mike: Webapps works in fact quite like this too, with subgroups working on different ideas. WebAppSec too. Mike West is prolific and also gets people implementaing things.

Mike: The model with going to Hixie worked often — yes there were some mistakes like AppCache, but there were

Alex: we have got by the heroic efforts of a few individuals — I would like standards much more boring.

Mike: Where to the bug fixes go when we find them? Also important question Hixie can fix bugs, and he does talk to implementers.

Alex: I remember the table sorting thing — we looked at the proposal and said it would not fly but our feedback was not taken into account

Hadley: Bad to have a single point of failure in the process.

Mike: Much less so now.
… we have an editor but we can have multiple editors
… The supposed division between whatwg and w3c never existed — I know — I was originally whatwg and many like me have feet on many places. don’tthink people were super happy with the way it worked in the whatwg — we do need to make a new workmode which focusses on real collaboration.
… I always try to get people to make tests and review them. There are only 5 people who review thos specs. Very hard to get people to write test cases.
… People don’t have time.
… We need people to contribute in reasonable chunks of time.

Alex: Important that powerful people don’t have the influence to bring in features without proper consideration and review. Browser makershave power. Should not implement things that seem a good idea at the time.
…The community group is a better model that allows review, and prioritizing new work.

DKA: So, should we tell devs about this new process, to use it for their new ideas? Is there a TAG role there?

Travis: Sounds very relevant

Mike: This is one idea … it isn’t baked .. and this is a multi-stakeholder consortium. Very early … not discussed with many place slike AB and AC.
… will be discussed in many places.

Hadley: role of TAG here? We don’t want to be in the critical path and slow things down.

Alex: Blink team now has a process for puting things into the product, which connects to the diagram on the board — trout simming upstream … The TAG should be involved with new work decisions at W3C in general.
...
… Let’s have some sort of meeting about this around the AC

### Interaction Events

Charles: A basic problem is the way in which user interaction happens is basically broken…
If you have links, buttons and forms - things moe or less work…
If you want moderately useful keyboard interaction, for example in particular — you often write a crap tonne of js which doesn’t quite work…

Tim: Like entering numbers on a over-secure bank site?

Ch: more like browser grabbing the keys which your web site want to use as key input
… what we try to do is to get devs to write interfaces for users… and most of the time they don't do a good job

Travis: Conflict between having useful keyboard shortcuts in the browser vs allowing devs to use absolutely everything

Ch: I know about this from the 90’s . The attention evnts TF wrk is an attempt to clean this up — a bit
… Work to define things with ARIA today is - you describe the roles, which is useful, then you again write a ton of broken JS.
… One approach is — to define richer things in ARIA which define behaviour and you ask the browsers write code which works in the enviroment.
...
…IS AcessKey good for you because you think it is available, or because it is easier for authiors, or because it doesn’t involve script?

Ch: All the above
…Keyboard navigation w/o the access key consists of moving the tab button to move though all the 500 buttons in your email page
…Opera didn’t do that — but then so many pages said “use the tab key” that Opera had to chnage

Charles: Access key is not brilliant but works

Léonie: Screen readers can pull out access keys — and tell a screen reader what is available.

Ch: Opera — pressing dot “.” would pop up a menu of access keys, which you could work from te menu or just use the key… Worked better than any other browser

Charles: Do not standradize keys which many pages use incommon, crate a higher level abstraction. Like “mark mail as spam” as a browser generated standard event

Alex: When designing a rich test editor, mapping the processing of keys was complex.
…There is a cintrioller which generates data which generates view — but I have access to none of these.

Alex: Does accesskey allow you to specify combinators?

Ch: It allows you to specify a space separeted list of characters, and it expects its chosen accelerator to be used with the first one on th list
…Interesting problem here is - how to simple devs write a simple system which works? Never much high end complex frameoworks.

Alex: Well, how about common ground between those use cases?
...
…The webapp should be able to request a set of controls — in some cases to be told it can’t get it
…The consistency should be primarily across web pages for the same browser, not across browsers for the same web page.
…ARIA things are more baked than random stuff in a new web app.
…and more baked than thinks in HTML.
…Does having things like “Copy” defined help? Having that extensible?
…Ideally you can define as many as you [the dev] wants.
…There is a conflict avoidance mechanism.

Travis: Can you talk about Intention events?

Ch: When you want to do text input, you say “input this text”, “select these things”… things like “mark as spam” “scroll ul” you would be able to collect the event from the borwser.
...
…In ARIA you have algos to find a name which you can present — say to sometime wondering what would happen when click on a button.
…The other question i whether the brower feeds back the application … the author on a good day knows the app better than the browser, and so the web app should be able to getthe info as to what is reserverd, so it can resolve conflicts.
...
Alex: Has anyone charted the path of a keystroke throughout the whole sequence of events?
...
Ch: I have used event.accesskey and it works at least in some browsers
...
Ch: Yes but not common across browsers

Some users like direct action, others prefer to know what will happen before they allow it to.
...
Ch: It is impossible to find key combos across all platforms
...
…The new spec is simpler — just “I want this content displayed over here”
…So each node contributes to the design, but the content is bullt outward from the templatwe to the page
…There is no way of having complex markup inserted specifically into a design.
...
… This implies about something which CSS has which has — this has nothing to into user space

Alex: Named flows do a version of intrinsic width based on container. You flow things into a container and wrap.

That is core to CSS, and ios not available to the rest of the system.

Remeber IHT in 2002 doing pagination and layout? They all call out for the power

Peter: Fragmentation and line breaks are different.

Alex: I am not opposed to having this in the platform — but now in 2015 there are common features with things ike

Peter: Multicolumns etc

Alex: .. which are not available to the user.

Regions exposes more of the problem …

Alan: An early version of regions defined flow control myuch more pwoerfully but we pulled iot out as it was common to lot sof other things too

Alex: If you have frag events, then there is some time, when we have promise delibvery etc etc end pof the layout cwork chunkc, time to move on.
… This seems to be a well-constructed design. Just need these events

Alan: People have used named flows on single box content.
…There is a processing model for distrubuting content amobne the boxes in the region chain

[discussion of detains of event timing sequence]

Alex: If we could allow the app to do more work to participate in the layout process, that would be useful
...
… When something has flowed into a different box associated woth a diff DOM, where does the event go?

Travis: I hope - go with original dom

Alan: Yes. You will get the whole stack of things underneath it, and when that is available, then you wil be able to make it so events follow the stack not the original DOM tree.

Alex: So is A flows into B does the shadow dom from B get replaced with that of A?
...
Alan: If you hav a flow from an element with its own children, it stays its own regular self.

Alan: No I meant [missed]
...
Alex: before and after pseudo-elements — do the come from name flow?

Alan: No from region chain
...
Alan: If named flow content has before and after pseudo-elements, they flow along the region change with everything else

peter: can we use flow from with before and after?

[Agreed: Too scary]

Alex: You only use same-origin IFrames here?

Travis: Yes, or you are pulling conetnt from one origi to another.

Alan: The region spec has the content keyworkd for this iframe.

Alan: Ther is a way in CSS to sy “don’t display this parent box, but just ignore it and display all its children”

http://drafts.csswg.org/css-display/#the-display-outside

Alan: Implemented in Firefox and Chrome maybe.
...
Alan: The overwhelming use case is to add more dom nodes to cope with the overflow. Which a lot of people in the CSS WG reckon is bad
…You make a region chain you think will hold will the stuff and then you add more boxes if yoy were wrong
…Much quicker in fact to pare off un-needed extras than to add more boxes.
...
Mark: The IETF recognize this as a major problem.
…There is a DHCP option to indicate where you need t log into a captive portal. The original discussions had not involved the operators. The
…solution was actually good but insufficiently discussed around
…So we had a great discussion of all of this
…Hotspot 2.0 is a new wifi roaming alliance spec — we discussed
...
…There will porbably be a working group around captive portals — it will be important to get the operators to come and be involved.

DKA: There is an arms race between the captive portal people trying to catpture people baut the apple-like software trying to automate the process or put it itno a specially constraine bowser context

[discussion of advertizing based business models etc]

Tim: Why do CPs bother intercepting and rerouting HTTS connections anyway?

Peter: They want to be able to show you something even if you start looking for https: something

Mark: We need to tell people who want to do the right thing — like a home or a university — how to do the right thing.

Peter: And we need to also talk about what haooens with the old providers who are not
