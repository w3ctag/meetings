...
[discussion on redecentralization of the web]
...
dka: there was some pushback: "hey, you're not lawers and are weighing on a subject you're not part of"...
(original request was for an "amicus brief"
...
hadley: government's power is in regulating behavior; W3c's power is in building things.
...
dwa: considering geoloc as we discussed at ExWebSummit.
...
dwa: Uh.. not sure we have. My point is that the model is a frame of reference for understanding the model that tim described.
...
dwa: so... obvoiusly it would not be appropriate for the TAG to take a stand on gambling (example). I think it is appropriate to say: the web is a tool for free speech.
...
Different Storage Technologies in web standards:
    ...
    * HTTP Caching - no API :(
[FYI, Mozilla Device Storage API](\url{https://developer.mozilla.org/en-US/docs/Web/API/Device\_Storage\_API)} (non-standard, only for privileged apps)
[Extensible Web sesssion on storage[\url{https://pad.w3ctag.org/p/EWS-20April2015-Breakout1.1.md}]
[An early Durable Storage proposal](\url{https://github.com/slightlyoff/StorageDurability/blob/master/explainer.md)}
[AnneVK’s concrete proposal for unified architecture for storage on the web](\url{https://wiki.whatwg.org/wiki/Storage)}
[navigator.connect API proposal](\url{http://mkruisselbrink.github.io/navigator-connect/)}
[navigator.connect explainer](\url{https://github.com/mkruisselbrink/navigator-connect/blob/gh-pages/explainer.md)}
...
Alex: It's hard for me to unpack all the roles I'm playign here.  What's likely to happen, and what's reasonable, and what the opportunities are:
   ...
   *     ... It's likely we'll struggle with the storage mechanismsm we have right now, because they're not integrated. Re the locking propblem: IndexDB has a similar problem. The Cache API has to preserve opacity for non-CORS cross-origin content.     ...It's attributed to your origin storage.  You can set whatever mode for the Fetch that you would like (CORS, no CORS). You'll receive a different type in return: a regular response, or an opaque response (which doesn't allow you to see hte content).
...
Topic: Future of the future of the TAG

To include: 

   * Composition of the TAG
       * AB proposal to expand the TAG. (Chaals) expand to 13 members - \url{https://lists.w3.org/Archives/Public/public-w3process/2015Mar/0029.html}
...
TOPIC: Powerful Features, Security \& Privacy review process
... with special guests Tantek, Nick Doty, and Mike West.

[Tantek's blog post on security](\url{http://tantek.com/2015/068/b1/security-towards-minimum-viable-web-platform} )
[Mike West's security review questionaire](\url{https://mikewest.github.io/spec-questionnaire/security-privacy/} )
...
[Principles of the Same-Origin Policy](\url{https://tools.ietf.org/html/rfc6454#section-3)} from RFC 6454, “The Web Origin Concept”
...
[Mozilla's Intent to Implement process](\url{https://wiki.mozilla.org/WebAPI/ExposureGuidelines#Intent\_to\_Implement)}
...
slightlyoff: I'm worried that this won't be part of a larger process...that it'll be disconnected from all the other things we ask them to consider
...
ndoty: I have a [fingerprinting draft](\url{http://w3c.github.io/fingerprinting-guidance/)}
...
mkwst: have you looked at the sub-origins proposal? (\url{http://blog.joelweinberger.us/2013/08/suborigins-for-privilege-separation-in.html)}
...
tnatek: looking forward to seeing the TAG repo URL - will be adding non-normative self-review to CSS3-UI: \url{http://dev.w3.org/csswg/css-ui-3/}


TOPIC: priviledged contexts (nee: powerful features) - \url{https://w3c.github.io/webappsec/specs/powerfulfeatures/}
...
[intent to deprecate HTTP](\url{https://groups.google.com/forum/#}!topic/mozilla.dev.platform/xaGffxAM-hs%5B1-25%5D)
...
TOPIC: Packaging! with special guest Ilya (\url{https://github.com/w3ctag/packaging-on-the-web)}
=======================================================================
...
[Web Performance Group draft charter](\url{https://w3c.github.io/charter-webperf/)}
\url{https://github.com/w3c/charter-webperf}
...
FYI in the WebDriver spec we have a [element displayed algorithm](\url{https://w3c.github.io/webdriver/webdriver-spec.html#dfn-element-displayed)} for ascertaining an element's visibility.
...
and [Web Security Experience, Indicators and Trust: Scope and Use Cases](\url{http://www.w3.org/TR/wsc-usecases/)} from 2008

<TAG returns from mandatory team building event>
<TAG departs for mandatory team building event>
